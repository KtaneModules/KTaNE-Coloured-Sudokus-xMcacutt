...
[Serializable]
public class XwingSudokuData
{
    public List<int> grid;
}

public class XwingSudokuScript : MonoBehaviour
{
    ...
    private XwingSudokuData _xwingSudoku;
    ...
    private void Start()
    {
        ...
        InitializeMaterials();
        _xwingSudoku = JsonConvert.DeserializeObject<List<XwingSudokuData>>(sudokuJson.text).OrderBy(_ => UnityEngine.Random.value).First();
    ...
    private IEnumerator GenerateSquares()
    {
        ...
        for (var row = 0; row < 9; row++)
        {
            for (var col = 0; col < 9; col++)
            {
                ...
                var value = _xwingSudoku.grid[index];
